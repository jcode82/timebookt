-- Limit availability block selection to the same UTC date as the appointment
-- to avoid picking blocks from other dates that share weekday/time.
...
begin
  ...
  if p_staff_id is not null then
    ...
    where ab.business_id = p_business_id
      ...
      and (ab.start_time at time zone 'utc')::date = (p_start_time at time zone 'utc')::date
...
begin
  ...
  if v_appointment.staff_id is not null then
    ...
    where ab.business_id = v_appointment.business_id
      ...
      and ab.day_of_week = v_day_of_week
      and (ab.start_time at time zone 'utc')::date = (p_new_start_time at time zone 'utc')::date
